...
describe("TRDEFIVault", function () {
  ...
  describe("Deposit / Withdraw", function () {
    ...
    it("Should not allow withdrawal of locked balance", async function () {
      ...
      await vault.createEvent("Test event?", "test", deadline, 0);
  ...
  describe("Event Creation", function () {
    it("Should create an event", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      const tx = await vault.createEvent("BTC > $100k tomorrow?", "kripto-gun", deadline, 95000);
    ...
    it("Should reject past deadlines", async function () {
      ...
      await expect(
        vault.createEvent("Past event?", "test", pastDeadline, 0)
  ...
  describe("Betting", function () {
    ...
    it("Should reject bets after deadline", async function () {
      ...
      await vault.createEvent("Short deadline?", "test", deadline, 0);
      const newEventId = 2;
  ...
  describe("Parimutuel Payout", function () {
    it("Should correctly distribute winnings (YES wins)", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      await vault.createEvent("BTC > $100k?", "kripto-gun", deadline, 95000);
    ...
    it("Should refund on DRAW (only one side)", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      await vault.createEvent("Only YES bets", "test", deadline, 0);
    ...
    it("Should handle multiple winners correctly", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      await vault.createEvent("Multi-winner test", "test", deadline, 0);
  ...
  describe("Platform Fees", function () {
    it("Should accumulate platform fees correctly", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      await vault.createEvent("Fee test", "test", deadline, 0);
    ...
    it("Should allow owner to withdraw fees", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      await vault.createEvent("Fee withdrawal test", "test", deadline, 0);
  ...
  describe("Security", function () {
    it("Should prevent non-resolver from resolving events", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      await vault.createEvent("Security test", "test", deadline, 0);
    ...
    it("Should prevent double claiming", async function () {
      const deadline = (await ethers.provider.getBlock("latest")).timestamp + 86400;
      await vault.createEvent("Double claim test", "test", deadline, 0);
